...
\bas
\text{Use } \sigma \in \Gamma(\mathcal{G}): r_\sigma(p) \coloneqq p \cdot \sigma_{\pi_{\mathcal{P}}(p)}
\eas
\end{frame}

\begin{frame}{Connection on $\mathcal{P}$: Idea}
\textcolor[rgb]{0,0.58,0}{$H$} a horizontal bundle of $\mathcal{P}$:
	\begin{figure}
	\begin{tikzpicture}[scale=1.2]
		...
		\draw [->] (4.41,-1) -- (4.71,-0.88);
		\draw [->] (4.41,1) -- (4.71,2);
		\draw [->,dotted,thick] (4.41,1) -- (4.71,1.12);
		\draw [dotted,thick] (4.71,2) -- (4.71,1.12);
		%Labels
		...
		\draw (5.25,1.56) node {$\thicksim \mathrm{D}_x\sigma$};
	\end{tikzpicture}
	\end{figure}
\end{frame}
